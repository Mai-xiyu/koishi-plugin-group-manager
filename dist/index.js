...
const ForbiddenSchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(false).description('启用违禁词检测'),
    words: koishi_1.Schema.array(String).default(DEFAULT_FORBIDDEN_WORDS).description('违禁词列表'),
    action: koishi_1.Schema.union(['warn', 'mute', 'kick']).default('mute').description('处理方式'),
    muteSeconds: koishi_1.Schema.number().default(600).description('禁言时长（秒）'),
    profanityApi: koishi_1.Schema.object({
        enabled: koishi_1.Schema.boolean().default(false).description('启用在线违禁词 API 检测'),
        endpoint: koishi_1.Schema.string().default('https://uapis.cn/api/v1/text/profanitycheck').description('违禁词检测 API 地址'),
    }).description('在线违禁词 API（使用 uapis.cn 敏感词检测服务）'),
}).description('违禁词检测');
const PenaltyLevelSchema = koishi_1.Schema.object({
    count: koishi_1.Schema.number().default(1).description('触发次数'),
    action: koishi_1.Schema.union(['warn', 'mute', 'kick']).default('mute').description('处罚方式'),
    muteSeconds: koishi_1.Schema.number().default(600).description('禁言时长（秒）'),
});
const PenaltySchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(true).description('启用多级处罚'),
    windowSeconds: koishi_1.Schema.number().default(3600).description('计数窗口（秒）'),
    levels: koishi_1.Schema.array(PenaltyLevelSchema).default([
        { count: 1, action: 'warn', muteSeconds: 0 },
        { count: 2, action: 'mute', muteSeconds: 600 },
        { count: 3, action: 'kick', muteSeconds: 0 },
    ]).description('处罚规则'),
}).description('多级处罚系统');
const SpamSchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(false).description('启用刷屏检测'),
    windowSeconds: koishi_1.Schema.number().default(10).description('统计窗口（秒）'),
    maxMessages: koishi_1.Schema.number().default(6).description('窗口内最大消息数'),
    muteSeconds: koishi_1.Schema.number().default(600).description('禁言时长（秒）'),
}).description('刷屏检测');
// 题库问题Schema - 支持多答案
const QuestionSchema = koishi_1.Schema.object({
    q: koishi_1.Schema.string().required().description('问题'),
    a: koishi_1.Schema.array(String).required().description('答案列表（多个答案均可通过）'),
});
const JoinVerifySchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(false).description('启用进群验证'),
    questionPool: koishi_1.Schema.array(QuestionSchema).default([
        { q: '请回答：1+1=?', a: ['2', '二', '贰'] },
    ]).description('题库（随机抽题，每题支持多个正确答案）'),
    timeoutSeconds: koishi_1.Schema.number().default(120).description('超时秒数'),
    kickOnFail: koishi_1.Schema.boolean().default(true).description('验证失败踢出群'),
}).description('进群验证');
const JoinNoticeSchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(false).description('启用入群通知'),
    template: koishi_1.Schema.string().default('欢迎 {user} 加入 {group}').description('消息模板 ({user}=用户ID, {group}=群名)'),
}).description('入群通知');
const LeaveNoticeSchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(false).description('启用退群通知'),
    template: koishi_1.Schema.string().default('{user} 已退出 {group}').description('消息模板 ({user}=用户ID, {group}=群名)'),
}).description('退群通知');
const AnnouncementSchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(false).description('启用群公告'),
    text: koishi_1.Schema.string().default('').description('公告内容'),
}).description('群公告');
const AtAllSchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(false).description('启用艾特全体'),
    cooldownSeconds: koishi_1.Schema.number().default(3600).description('冷却时间（秒）'),
}).description('艾特全体');
const FileManageSchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(false).description('启用群文件管理'),
    allowExtensions: koishi_1.Schema.array(String).default([]).description('允许的文件扩展名（空=全部允许）'),
}).description('群文件管理');
const KeywordAnnounceSchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(false).description('启用此规则'),
    keywords: koishi_1.Schema.array(String).default([]).description('触发关键词列表'),
    message: koishi_1.Schema.string().default('').description('触发后发送的公告内容'),
    cooldownSeconds: koishi_1.Schema.number().default(300).description('触发冷却（秒）'),
});
const ScheduleAnnounceSchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(false).description('启用此规则'),
    intervalMinutes: koishi_1.Schema.number().default(60).description('间隔分钟'),
    message: koishi_1.Schema.string().default('').description('公告内容'),
});
const WelcomeGuideSchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(false).description('启用欢迎引导'),
    text: koishi_1.Schema.string().default('欢迎加入！请先阅读群公告并遵守规则。').description('欢迎文字'),
    image: koishi_1.Schema.string().default('').description('欢迎图片 URL（可选）'),
}).description('欢迎引导');
const AutoRecallSchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(true).description('自动撤回违规消息'),
}).description('自动撤回');
const AppealSchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(true).description('启用申诉功能'),
    notifyManagers: koishi_1.Schema.boolean().default(true).description('通知群管理员'),
}).description('申诉功能');
const WebSchema = koishi_1.Schema.object({
    enabled: koishi_1.Schema.boolean().default(true).description('启用管理网站'),
    host: koishi_1.Schema.string().default('').description('公网地址（留空自动检测）支持：域名、域名:端口、IP:端口。如：example.com、example.com:8080、[2409:xxxx::1]:5140'),
    autoDetectIP: koishi_1.Schema.boolean().default(true).description('自动检测公网 IP（仅在 host 留空时生效）'),
    preferIPv6: koishi_1.Schema.boolean().default(true).description('优先使用 IPv6（仅自动检测时生效）'),
    tokenExpireSeconds: koishi_1.Schema.number().default(3600).description('登录有效期（秒）'),
}).description('网站控制（端口由 @koishijs/plugin-server 控制，手动指定 host 时可在地址中包含端口）');
// 群配置 Schema - 使用 collapse 让其以树形结构展示
const GroupSchema = koishi_1.Schema.intersect([
    koishi_1.Schema.object({
        groupId: koishi_1.Schema.string().required().description('群号/频道 ID'),
        enabled: koishi_1.Schema.boolean().default(true).description('启用本群管理'),
        autoApprove: koishi_1.Schema.boolean().default(false).description('自动同意入群申请'),
    }).description('基本设置'),
    koishi_1.Schema.object({
        whitelist: koishi_1.Schema.array(String).default([]).description('免检白名单（不受规则限制）'),
        blacklist: koishi_1.Schema.array(String).default([]).description('黑名单（自动踢出）'),
        graylist: koishi_1.Schema.array(String).default([]).description('灰名单（重点监控，违禁词检测更严格）'),
    ...
    koishi_1.Schema.object({
        ...
        atAll: AtAllSchema,
        keywordAnnounce: koishi_1.Schema.array(KeywordAnnounceSchema).default([]).description('关键词触发公告'),
        scheduleAnnounce: koishi_1.Schema.array(ScheduleAnnounceSchema).default([]).description('定时公告'),
    }).description('公告设置'),
    koishi_1.Schema.object({
...
exports.Config = koishi_1.Schema.intersect([
    koishi_1.Schema.object({
        admins: koishi_1.Schema.array(String).default([]).description('机器人管理员QQ号列表（可多个，拥有最高权限）'),
        adminAuthority: koishi_1.Schema.union([1, 2, 3]).default(2).description('命令权限等级：1=所有人, 2=管理员及以上, 3=仅群主'),
        configFile: koishi_1.Schema.string().default('data/qq-group-manager.json').description('群配置存储 JSON 路径'),
        recordsFile: koishi_1.Schema.string().default('data/qq-group-manager-records.json').description('处罚/申诉记录存储路径'),
    }).description('基本设置'),
    koishi_1.Schema.object({
        groups: koishi_1.Schema.array(GroupSchema).default([]).description('点击添加群配置'),
    }).description('群管理配置'),
    koishi_1.Schema.object({
        web: WebSchema,
    }).description('Web 控制台'),
    koishi_1.Schema.object({
        imageModeration: koishi_1.Schema.object({
            enabled: koishi_1.Schema.boolean().default(false).description('启用敏感图检测'),
            endpoint: koishi_1.Schema.string().default('').description('图片检测 API 地址'),
            apiKey: koishi_1.Schema.string().default('').description('API Key（可选）'),
            threshold: koishi_1.Schema.number().default(0.7).description('命中阈值（0-1）'),
        }).description('敏感图检测配置'),
    }).description('敏感图检测'),
]);
function apply(ctx, config) {
    ...
    const getRoleLevel = (session) => {
        ...
        const role = ((_a = session === null || session === void 0 ? void 0 : session.member) === null || _a === void 0 ? void 0 : _a.role) || ((_c = (_b = session === null || session === void 0 ? void 0 : session.event) === null || _b === void 0 ? void 0 : _b.sender) === null || _c === void 0 ? void 0 : _c.role) || '';
        if (role === 'owner')
            return 3; // 群主
        if (role === 'admin')
            return 2; // 管理员
    ...
    const getRoleLevelAsync = async (session) => {
        ...
        const groupId = String((session === null || session === void 0 ? void 0 : session.guildId) || '');
        if (((_a = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _a === void 0 ? void 0 : _a.getGroupMemberInfo) && groupId && userId) {
            try {
                const info = await bot.internal.getGroupMemberInfo(groupId, userId, false);
                const role = (info === null || info === void 0 ? void 0 : info.role) || '';
        ...
        // 最后使用 session 中的信息
        const sessionRole = ((_b = session === null || session === void 0 ? void 0 : session.member) === null || _b === void 0 ? void 0 : _b.role) || ((_d = (_c = session === null || session === void 0 ? void 0 : session.event) === null || _c === void 0 ? void 0 : _c.sender) === null || _d === void 0 ? void 0 : _d.role) || 'member';
    ...
    const requireManage = async (session, channelId) => {
        var _a, _b, _c, _d, _e;
        const level = Number((_a = config.adminAuthority) !== null && _a !== void 0 ? _a : 2);
        if (level <= 1)
            return true;
    ...
    const cleanupRequests = () => {
        ...
        for (const [id, req] of pendingRequests) {
            if (now - (req.timestamp || now) > 24 * 60 * 60 * 1000)
    ...
    const loadRecords = async () => {
        try {
            ...
            const content = await fs_1.promises.readFile(fp, 'utf8');
    ...
    const saveRecords = async () => {
        try {
            const fp = resolveRecordsFile();
            await fs_1.promises.mkdir(path_1.default.dirname(fp), { recursive: true });
            await fs_1.promises.writeFile(fp, JSON.stringify(records, null, 2), 'utf8');
        }
        catch (e) {
    ...
    const getGroup = (groupId) => {
        if (!Array.isArray(config.groups))
            config.groups = [];
        return config.groups.find((g) => String(g === null || g === void 0 ? void 0 : g.groupId) === String(groupId));
    };
    const ensureGroup = (groupId) => {
        if (!Array.isArray(config.groups))
            config.groups = [];
        let group = config.groups.find((g) => String(g === null || g === void 0 ? void 0 : g.groupId) === String(groupId));
        if (!group) {
            group = {
    ...
    const onebotCall = async (bot, action, params) => {
        if (!bot)
            return false;
        try {
    ...
    const isManager = (group, userId) => {
        const list = Array.isArray(group === null || group === void 0 ? void 0 : group.managers) ? group.managers.map(String) : [];
        return list.includes(String(userId));
    };
    const inList = (list, userId) => Array.isArray(list) && list.map(String).includes(String(userId));
    const recordPunish = async (data) => {
        records.punishments.push({ ...data, ts: Date.now() });
        await saveRecords();
    };
    const recordAppeal = async (data) => {
        records.appeals.push({ ...data, ts: Date.now() });
        await saveRecords();
    };
    const checkImage = async (url) => {
        var _a, _b, _c, _d, _e;
        if (!((_a = config.imageModeration) === null || _a === void 0 ? void 0 : _a.enabled) || !config.imageModeration.endpoint)
            return false;
        try {
            const res = await (0, node_fetch_1.default)(config.imageModeration.endpoint, {
                method: 'POST',
                headers: {
            ...
            if ((json === null || json === void 0 ? void 0 : json.nsfw) === true)
                return true;
            const score = Number((_d = (_c = (_b = json === null || json === void 0 ? void 0 : json.score) !== null && _b !== void 0 ? _b : json === null || json === void 0 ? void 0 : json.nsfw_score) !== null && _c !== void 0 ? _c : json === null || json === void 0 ? void 0 : json.prob) !== null && _d !== void 0 ? _d : 0);
            const threshold = Number((_e = config.imageModeration.threshold) !== null && _e !== void 0 ? _e : 0.7);
    ...
    const checkProfanityApi = async (text, groupForbidden) => {
        const apiConfig = groupForbidden === null || groupForbidden === void 0 ? void 0 : groupForbidden.profanityApi;
        if (!(apiConfig === null || apiConfig === void 0 ? void 0 : apiConfig.enabled) || !apiConfig.endpoint) {
            return { hit: false, words: [] };
        }
        try {
            const res = await (0, node_fetch_1.default)(apiConfig.endpoint, {
                method: 'POST',
            ...
            if ((json === null || json === void 0 ? void 0 : json.status) === 'forbidden' && Array.isArray(json === null || json === void 0 ? void 0 : json.forbidden_words) && json.forbidden_words.length > 0) {
    ...
    const saveMessageStats = async () => {
        ...
        statsSaveTimer = setTimeout(async () => {
            try {
                ...
                await fs_1.promises.writeFile(fp, JSON.stringify(obj, null, 2), 'utf8');
            }
            catch (e) {
    ...
    // 更新消息统计
    const updateMessageStats = (groupId, userId) => {
    ...
    ctx.middleware(async (session, next) => {
        ...
        const applyPenalty = async (reason) => {
            ...
            const now = Date.now();
            let action = ((_a = group.forbidden) === null || _a === void 0 ? void 0 : _a.action) || 'warn';
            let muteSeconds = Number(((_b = group.forbidden) === null || _b === void 0 ? void 0 : _b.muteSeconds) || 600);
            let count = 1;
            // 多级处罚逻辑
            if ((_c = group.penalty) === null || _c === void 0 ? void 0 : _c.enabled) {
                ...
                const sortedLevels = levels.slice().sort((a, b) => Number(b.count || 1) - Number(a.count || 1));
                const level = sortedLevels.find((l) => count >= Number(l.count || 1));
                if (level) {
                    action = level.action;
            ...
            // 自动撤回
            if (((_d = group.autoRecall) === null || _d === void 0 ? void 0 : _d.enabled) && session.messageId) {
                try {
        ...
        if (((_a = group.forbidden) === null || _a === void 0 ? void 0 : _a.enabled) && textContent) {
            ...
            if ((_c = (_b = group.forbidden) === null || _b === void 0 ? void 0 : _b.profanityApi) === null || _c === void 0 ? void 0 : _c.enabled) {
        ...
        // 刷屏检测
        if (((_d = group.spam) === null || _d === void 0 ? void 0 : _d.enabled) && session.userId) {
        ...
        if (inList(group.graylist || [], userId) && content) {
            const words = Array.isArray((_e = group.forbidden) === null || _e === void 0 ? void 0 : _e.words) ? group.forbidden.words : DEFAULT_FORBIDDEN_WORDS;
        ...
        // 敏感图检测
        if (((_f = config.imageModeration) === null || _f === void 0 ? void 0 : _f.enabled) && Array.isArray(session.elements)) {
            const imgs = session.elements.filter((el) => (el === null || el === void 0 ? void 0 : el.type) === 'image');
            for (const img of imgs) {
                const url = String(((_g = img === null || img === void 0 ? void 0 : img.attrs) === null || _g === void 0 ? void 0 : _g.url) || ((_h = img === null || img === void 0 ? void 0 : img.attrs) === null || _h === void 0 ? void 0 : _h.src) || '');
                if (!url)
                    continue;
        ...
        // 群文件管理（粗略检测 file 元素）
        if (((_j = group.fileManage) === null || _j === void 0 ? void 0 : _j.enabled) && Array.isArray(session.elements)) {
            const allow = new Set((group.fileManage.allowExtensions || []).map((s) => s.toLowerCase()));
            const hasFile = session.elements.find((el) => (el === null || el === void 0 ? void 0 : el.type) === 'file');
            if (hasFile) {
                const name = String(((_k = hasFile === null || hasFile === void 0 ? void 0 : hasFile.attrs) === null || _k === void 0 ? void 0 : _k.name) || '');
                const ext = name.includes('.') ? name.split('.').pop().toLowerCase() : '';
                if (allow.size && (!ext || !allow.has(ext))) {
                    await session.send('该群禁止发送此类型文件。');
                    if (((_l = group.autoRecall) === null || _l === void 0 ? void 0 : _l.enabled) && session.messageId) {
                        try {
                            await session.bot.deleteMessage(session.channelId, session.messageId);
                        }
                        catch { }
        ...
        if (Array.isArray(group.keywordAnnounce) && content) {
            for (let idx = 0; idx < group.keywordAnnounce.length; idx++) {
                ...
                if (!kws.length)
                    continue;
                const hit = kws.find((k) => k && content.includes(k));
                if (!hit)
                    continue;
    ...
    ctx.on('guild-request', async (session) => {
        ...
        const flag = String(session.messageId || ((_b = (_a = session.event) === null || _a === void 0 ? void 0 : _a._data) === null || _b === void 0 ? void 0 : _b.flag) || '');
        const subType = String(((_d = (_c = session.event) === null || _c === void 0 ? void 0 : _c._data) === null || _d === void 0 ? void 0 : _d.sub_type) || 'invite');
        logger.info(`收到 guild-request 事件 - 群:${groupId} 类型:${subType} 用户:${session.userId} flag:${flag}`);
        // 自动同意入群申请逻辑（用户主动申请加群）
        if (subType === 'add') {
            const groupConfig = getGroup(groupId);
            logger.debug(`群 ${groupId} autoApprove 配置: ${groupConfig === null || groupConfig === void 0 ? void 0 : groupConfig.autoApprove}`);
            if (groupConfig && groupConfig.autoApprove === true) {
                try {
                    // OneBot v11 API: set_group_add_request
                    const bot = session.bot;
                    if ((_e = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _e === void 0 ? void 0 : _e.setGroupAddRequest) {
                        await bot.internal.setGroupAddRequest(flag, subType, true, '');
                        logger.info(`已自动同意用户 ${session.userId} 加入群 ${groupId}`);
                        return;
                    }
                    else if ((_f = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _f === void 0 ? void 0 : _f.set_group_add_request) {
        ...
        if (approvedGroups.has(groupId)) {
            try {
                if ((_h = (_g = session.bot) === null || _g === void 0 ? void 0 : _g.internal) === null || _h === void 0 ? void 0 : _h.setGroupAddRequest) {
                    await session.bot.internal.setGroupAddRequest(flag, subType, true, '');
    ...
    ctx.on('friend-request', async (session) => {
        ...
        const userId = String(session.userId || '');
        const flag = String(session.messageId || ((_b = (_a = session.event) === null || _a === void 0 ? void 0 : _a._data) === null || _b === void 0 ? void 0 : _b.flag) || '');
        const comment = String(((_d = (_c = session.event) === null || _c === void 0 ? void 0 : _c._data) === null || _d === void 0 ? void 0 : _d.comment) || '无');
        ...
        for (const adminId of admins) {
            try {
                await session.bot.sendPrivateMessage(adminId, `👤 收到加好友请求\n━━━━━━━━━━━━━━━\n请求ID: ${requestId}\n申请人: ${userId}\n验证消息: ${comment}\n━━━━━━━━━━━━━━━\n回复「同意 ${requestId}」或「拒绝 ${requestId}」`);
            }
            catch (e) {
                logger.warn(`无法通知管理员 ${adminId}: ${e.message}`);
            }
    ...
    ctx.command('同意 <requestId:string> [remark:text]', '同意入群/加好友请求（仅管理员私聊）')
        ...
        if (!request)
            return `未找到请求ID: ${requestId}\n使用「查看邀请」查看所有待处理的请求`;
        // 处理好友请求
        if (request.type === 'friend-request') {
            try {
                const bot = session.bot;
                if ((_a = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _a === void 0 ? void 0 : _a.setFriendAddRequest) {
                    await bot.internal.setFriendAddRequest(request.flag, true, remark || '');
                }
                else if ((_b = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _b === void 0 ? void 0 : _b.set_friend_add_request) {
        ...
        // 处理群邀请
        try {
            if ((_c = session.bot.internal) === null || _c === void 0 ? void 0 : _c.setGroupAddRequest) {
                await session.bot.internal.setGroupAddRequest(request.flag, request.subType, true, '');
            }
            else if ((_f = (_e = (_d = request.session) === null || _d === void 0 ? void 0 : _d.bot) === null || _e === void 0 ? void 0 : _e.internal) === null || _f === void 0 ? void 0 : _f.setGroupAddRequest) {
    ...
    ctx.command('退群 <groupId:string>', '让机器人退出指定群聊（仅管理员）')
        ...
        if (!groupId || !/^\d+$/.test(groupId))
            return '请输入正确的群号（纯数字）';
        try {
            if ((_a = session.bot.internal) === null || _a === void 0 ? void 0 : _a.setGroupLeave) {
    ...
    // ================= 头衔管理 =================
    // 检查是否为群主
    const isGroupOwner = async (bot, groupId, userId) => {
        var _a;
        try {
            if ((_a = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _a === void 0 ? void 0 : _a.getGroupMemberInfo) {
                const info = await bot.internal.getGroupMemberInfo(groupId, userId, false);
                return (info === null || info === void 0 ? void 0 : info.role) === 'owner';
            }
        }
        catch { }
        return false;
    ...
    // 检查用户是否是群管理员或群主
    const isGroupAdmin = async (bot, groupId, userId) => {
        var _a;
        try {
            if ((_a = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _a === void 0 ? void 0 : _a.getGroupMemberInfo) {
                const info = await bot.internal.getGroupMemberInfo(groupId, userId, false);
                return (info === null || info === void 0 ? void 0 : info.role) === 'owner' || (info === null || info === void 0 ? void 0 : info.role) === 'admin';
            }
        }
        catch { }
        return false;
    ...
    ctx.command('qqgm.申请头衔 <text:string>', '申请专属头衔')
        ...
        try {
            await onebot.setGroupSpecialTitle(session.guildId, session.userId, text);
            return '✅ 已经改好啦~';
        }
        catch (e) {
    ...
    ctx.command('qqgm.修改头衔 <user:user> <text:string>', '修改他人头衔（需权限）')
        ...
        try {
            await onebot.setGroupSpecialTitle(session.guildId, qqnum, text);
            return '✅ 已经改好啦~';
        }
        catch (e) {
    ...
    const parseTimeString = (timeStr) => {
        ...
        if (match) {
            ...
            const unit = match[2];
            if (unit.match(/秒|s|sec/i))
                return Math.floor(num);
            if (unit.match(/分|m|min/i))
                return Math.floor(num * 60);
            if (unit.match(/时|小时|h|hour/i))
                return Math.floor(num * 3600);
            if (unit.match(/天|日|d|day/i))
    ...
    ctx.command('qqgm.禁言 [user:string] [time:text]', '禁言群成员')
        ...
        const bot = session.bot;
        try {
            if ((_a = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _a === void 0 ? void 0 : _a.setGroupBan) {
                await bot.internal.setGroupBan(groupId, targetUserId, seconds);
            }
            else if ((_b = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _b === void 0 ? void 0 : _b.set_group_ban) {
...
时间格式支持省略：
  ...
  - 2024年1月1日12时30分（完整格式）`)
        ...
        if (!startDate)
            return '无法解析开始时间，请使用格式如：12时30分、1日12时、1月1日12时30分';
        if (!endDate)
            return '无法解析结束时间，请使用格式如：12时30分、1日12时、1月1日12时30分';
        ...
        try {
            // 获取群消息历史
            let messages = [];
            if ((_a = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _a === void 0 ? void 0 : _a.getGroupMsgHistory) {
                // 尝试获取消息历史（部分 OneBot 实现支持）
                const history = await bot.internal.getGroupMsgHistory(groupId, 0);
                messages = (history === null || history === void 0 ? void 0 : history.messages) || [];
            }
            else if ((_b = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _b === void 0 ? void 0 : _b.get_group_msg_history) {
            ...
            const toRecall = messages.filter((msg) => {
                var _a;
                const senderId = String(((_a = msg === null || msg === void 0 ? void 0 : msg.sender) === null || _a === void 0 ? void 0 : _a.user_id) || (msg === null || msg === void 0 ? void 0 : msg.user_id) || '');
                const msgTime = Number((msg === null || msg === void 0 ? void 0 : msg.time) || 0);
            ...
            for (const msg of toRecall) {
                const msgId = msg === null || msg === void 0 ? void 0 : msg.message_id;
                if (!msgId)
                    continue;
                try {
                    if ((_c = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _c === void 0 ? void 0 : _c.deleteMsg) {
                        await bot.internal.deleteMsg(msgId);
                        successCount++;
                    }
                    else if ((_d = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _d === void 0 ? void 0 : _d.delete_msg) {
    ...
    ctx.on('guild-member-added', async (session) => {
        ...
        const userId = String(session.userId || '');
        const groupName = String(session.guildName || session.guildId);
        // 黑名单用户直接踢
        if (inList(group.blacklist || [], userId)) {
        ...
        // 入群通知
        if ((_a = group.joinNotice) === null || _a === void 0 ? void 0 : _a.enabled) {
        ...
        // 入群验证
        if ((_b = group.joinVerify) === null || _b === void 0 ? void 0 : _b.enabled) {
            ...
            if (pool.length === 0) {
                ...
                try {
                    await session.send(`${koishi_1.h.at(userId)} ${q}`);
                }
                catch (e) {
        ...
        // 欢迎引导
        if ((_c = group.welcomeGuide) === null || _c === void 0 ? void 0 : _c.enabled) {
            logger.info(`发送欢迎引导 - 群:${session.guildId} 用户:${userId}`);
            try {
                if (group.welcomeGuide.image) {
                    await session.send(koishi_1.h.image(group.welcomeGuide.image));
                }
    ...
    ctx.on('guild-member-removed', async (session) => {
        ...
        if ((_a = group.leaveNotice) === null || _a === void 0 ? void 0 : _a.enabled) {
    ...
    // 入群验证中间件 - 需要在主中间件之前执行
    ctx.middleware(async (session, next) => {
        if (session.platform !== 'onebot' || !session.guildId || !session.userId)
            return next();
        const key = `${session.guildId}:${session.userId}`;
...
<body>
  <div class="container">
    ...
    ctx.command('qqgm.config <path:string> <operation:string> [value:text]', '动态修改群组配置')
        ...
        const setValue = (obj, parts, value) => {
            let current = obj;
    ...
    ctx.command('qqgm.atall <text:text>', '群艾特全体')
        ...
        const group = ensureGroup(session.guildId);
        if (!((_a = group.atAll) === null || _a === void 0 ? void 0 : _a.enabled))
            return '本群未启用艾特全体功能。';
        ...
        await session.send(koishi_1.h.at('all') + (text ? ` ${text}` : ''));
        return;
    });
    // === Web 控制：简单登录与配置接口 ===
    const webPasswords = new Map();
    const genWebPassword = () => crypto_1.default.randomBytes(4).toString('hex');
    // 暴露给 Web 模块
    ctx.qqgmWebPasswords = webPasswords;
    // 获取永久密码（从配置中）
    const getAdminPassword = (groupId, userId) => {
        const list = config.adminPasswords || [];
        const item = list.find((p) => p.groupId === groupId && p.userId === userId);
        return (item === null || item === void 0 ? void 0 : item.password) || null;
    };
    // 设置永久密码
    const setAdminPassword = async (groupId, userId, password) => {
        if (!config.adminPasswords) {
            config.adminPasswords = [];
        }
        const list = config.adminPasswords;
        const existingIdx = list.findIndex((p) => p.groupId === groupId && p.userId === userId);
        // 使用 sha256 加密存储
        const hashedPwd = crypto_1.default.createHash('sha256').update(password).digest('hex');
        if (existingIdx >= 0) {
    ...
    // 删除永久密码
    const removeAdminPassword = async (groupId, userId) => {
        if (!config.adminPasswords)
            return;
        const list = config.adminPasswords;
        const idx = list.findIndex((p) => p.groupId === groupId && p.userId === userId);
        if (idx >= 0) {
            list.splice(idx, 1);
            await doSaveConfig();
        }
    ...
    // 验证永久密码
    const verifyAdminPassword = (groupId, userId, password) => {
        const stored = getAdminPassword(groupId, userId);
        if (!stored)
            return false;
        const hashedInput = crypto_1.default.createHash('sha256').update(password).digest('hex');
        return stored === hashedInput;
    };
    // 暴露永久密码相关函数给 Web 模块
    ctx.qqgmAdminPassword = {
    ...
    const getWebUrl = () => {
        ...
        if (webConfig.autoDetectIP !== false) {
            const detected = (0, utils_1.detectPublicAddress)(webConfig.preferIPv6 !== false);
            if (detected.address) {
                ...
                let port = 5140;
                try {
                    if ((_b = (_a = ctx.server) === null || _a === void 0 ? void 0 : _a.config) === null || _b === void 0 ? void 0 : _b.port) {
                        port = ctx.server.config.port;
                    }
                }
                catch { }
    ...
    ctx.command('qqgm.appeal <text:text>', '提交申诉')
        ...
        if (!session.guildId || session.platform !== 'onebot')
            return '仅限群聊。';
        const group = getGroup(session.guildId);
        if (!group || ((_a = group.appeal) === null || _a === void 0 ? void 0 : _a.enabled) === false)
            return '本群未启用申诉。';
        const item = { groupId: session.guildId, userId: String(session.userId || ''), text };
        await recordAppeal(item);
        if (((_b = group.appeal) === null || _b === void 0 ? void 0 : _b.notifyManagers) && Array.isArray(group.managers)) {
            for (const mid of group.managers) {
                try {
                    await session.bot.sendPrivateMessage(mid, `申诉来自 ${item.userId}：${text}`);
                }
                catch { }
            }
        }
    ...
    const resetSchedules = () => {
        ...
        config.groups.forEach((g) => {
            ...
            rules.forEach((r) => {
                if (!(r === null || r === void 0 ? void 0 : r.enabled) || !r.message)
                    return;
            ...
            if (timers.length)
                scheduleTimers.set(String(g.groupId), timers);
        });
    };
    ctx.on('ready', async () => {
        // 确保 groups 数组已初始化
        ...
        try {
            ...
            const json = JSON.parse(content);
            if ((json === null || json === void 0 ? void 0 : json.adminPasswords) && Array.isArray(json.adminPasswords)) {
                config.adminPasswords = json.adminPasswords;
    ...
    const doSaveConfig = async () => {
        try {
            ...
            const obj = {
                ...
                adminPasswords: Array.isArray(config.adminPasswords) ? config.adminPasswords : []
            };
    ...
    (0, admin_1.registerAdminCommands)(ctx, config, logger, requireManage, ensureGroup);
    // Web Token 存储 - 在主模块创建，避免配置更新时被清空
    const webTokens = new Map();
    // 注册 Web 管理系统
    if (((_a = config.web) === null || _a === void 0 ? void 0 : _a.enabled) !== false) {
        (0, web_1.registerWebRoutes)(ctx, config, logger, getGroup, ensureGroup, records, doSaveConfig, getRoleLevelAsync, webTokens, webPasswords);
    }
    if (!server && ((_b = config.web) === null || _b === void 0 ? void 0 : _b.enabled) !== false) {
