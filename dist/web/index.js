...
// 生成 Token
const genToken = () => crypto_1.default.randomBytes(24).toString('hex');
// 前端 HTML 页面 - 极致美化版
...
`;
function registerWebRoutes(ctx, config, logger, getGroup, ensureGroup, records, doSaveConfig, getRoleLevelAsync, tokens, webPasswords) {
    ...
    server.post('/qqgm/api/login', async (kctx) => {
        var _a;
        const { groupId, userId, password } = kctx.request.body || {};
        if (!groupId || !userId || !password)
            return kctx.body = { ok: false, message: '参数不完整' };
        ...
        if (!isValidPassword)
            return kctx.body = { ok: false, message: '密码错误或已过期' };
        let role = 'admin';
        if (Array.isArray(config.admins) && config.admins.includes(String(userId))) {
            ...
            if ((_a = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _a === void 0 ? void 0 : _a.getGroupMemberInfo) {
                try {
                    ...
                    if ((info === null || info === void 0 ? void 0 : info.role) === 'owner')
                        role = 'owner';
                    else if ((info === null || info === void 0 ? void 0 : info.role) === 'admin')
                        role = 'admin';
                    else
                        return kctx.body = { ok: false, message: '您不是群管理员' };
                }
                catch (e) { }
            }
        }
    ...
    server.get('/qqgm/api/hasPassword', async (kctx) => {
        const token = String(kctx.request.headers['x-qqgm-token'] || '');
        const data = verifyToken(token);
        if (!data)
            return kctx.body = { ok: false, message: '未登录' };
    ...
    server.get('/qqgm/api/config', async (kctx) => {
        var _a, _b;
        const token = String(kctx.request.headers['x-qqgm-token'] || '');
        const data = verifyToken(token);
        if (!data)
        ...
        let groupName = '未命名群聊';
        const bot = ctx.bots.find(b => b.platform === 'onebot');
        if ((_a = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _a === void 0 ? void 0 : _a.getGroupInfo) {
            try {
                const groupInfo = await bot.internal.getGroupInfo(data.groupId, false);
                if (groupInfo === null || groupInfo === void 0 ? void 0 : groupInfo.group_name)
        ...
        // 获取机器人在该群的角色（判断是否群主）
        let botRole = 'member';
        if (((_b = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _b === void 0 ? void 0 : _b.getGroupMemberInfo) && (bot === null || bot === void 0 ? void 0 : bot.selfId)) {
            try {
                const botInfo = await bot.internal.getGroupMemberInfo(data.groupId, bot.selfId, false);
                botRole = (botInfo === null || botInfo === void 0 ? void 0 : botInfo.role) || 'member';
            }
            catch { }
        }
        return kctx.body = { ok: true, group, records: groupRecords, role: data.role, groupName, botRole };
    });
    server.post('/qqgm/api/config', async (kctx) => {
        var _a;
        const token = String(kctx.request.headers['x-qqgm-token'] || '');
        const data = verifyToken(token);
        if (!data)
        ...
        try {
            await doSaveConfig();
            if ((_a = ctx.scope) === null || _a === void 0 ? void 0 : _a.update)
                ctx.scope.update(config, true);
            return kctx.body = { ok: true };
        }
        catch (e) {
            return kctx.body = { ok: false, message: '保存失败' };
        }
    });
    // 获取群成员列表
    server.get('/qqgm/api/members', async (kctx) => {
        var _a;
        const token = String(kctx.request.headers['x-qqgm-token'] || '');
        const data = verifyToken(token);
        if (!data)
            return kctx.body = { ok: false, message: '未登录' };
        const bot = ctx.bots.find(b => b.platform === 'onebot');
        if (!((_a = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _a === void 0 ? void 0 : _a.getGroupMemberList))
            return kctx.body = { ok: false, message: 'Bot 不可用' };
        try {
    ...
    server.post('/qqgm/api/members/kick', async (kctx) => {
        var _a;
        const token = String(kctx.request.headers['x-qqgm-token'] || '');
        const data = verifyToken(token);
        if (!data)
        ...
        const bot = ctx.bots.find(b => b.platform === 'onebot');
        if (!((_a = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _a === void 0 ? void 0 : _a.setGroupKick))
            return kctx.body = { ok: false, message: 'Bot 不可用' };
        for (const userId of userIds)
            try {
                await bot.internal.setGroupKick(data.groupId, userId, false);
            }
            catch { }
        return kctx.body = { ok: true, message: '操作完成' };
    });
    server.post('/qqgm/api/members/mute', async (kctx) => {
        var _a;
        const token = String(kctx.request.headers['x-qqgm-token'] || '');
        const data = verifyToken(token);
        if (!data)
        ...
        for (const userId of userIds)
            try {
                await bot.internal.setGroupBan(data.groupId, userId, duration);
            }
            catch { }
        return kctx.body = { ok: true, message: '操作完成' };
    });
    server.post('/qqgm/api/members/title', async (kctx) => {
        var _a;
        const token = String(kctx.request.headers['x-qqgm-token'] || '');
        const data = verifyToken(token);
        if (!data)
        ...
        const bot = ctx.bots.find(b => b.platform === 'onebot');
        if (!((_a = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _a === void 0 ? void 0 : _a.setGroupSpecialTitle))
            return kctx.body = { ok: false, message: 'Bot 不可用' };
        for (const userId of userIds)
            try {
                await bot.internal.setGroupSpecialTitle(data.groupId, userId, title || '', -1);
            }
            catch { }
        return kctx.body = { ok: true, message: '操作完成' };
    });
    server.post('/qqgm/api/members/card', async (kctx) => {
        var _a;
        const token = String(kctx.request.headers['x-qqgm-token'] || '');
        const data = verifyToken(token);
        if (!data)
        ...
        for (const userId of userIds)
            try {
                await bot.internal.setGroupCard(data.groupId, userId, card || '');
            }
            catch { }
        return kctx.body = { ok: true, message: '操作完成' };
    });
    server.post('/qqgm/api/group/muteAll', async (kctx) => {
        var _a;
        const token = String(kctx.request.headers['x-qqgm-token'] || '');
        const data = verifyToken(token);
        if (!data)
        ...
        if (!((_a = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _a === void 0 ? void 0 : _a.setGroupWholeBan))
            return kctx.body = { ok: false, message: 'Bot 不可用' };
        try {
            await bot.internal.setGroupWholeBan(data.groupId, enable);
            return kctx.body = { ok: true };
        }
        catch (e) {
            return kctx.body = { ok: false, message: String(e) };
        }
    });
    server.get('/qqgm/api/group/mutedList', async (kctx) => {
        var _a;
        const token = String(kctx.request.headers['x-qqgm-token'] || '');
        const data = verifyToken(token);
        if (!data)
        ...
        try {
            ...
            const muted = (members || []).filter((m) => m.shut_up_timestamp && m.shut_up_timestamp > now)
                .map((m) => ({ userId: String(m.user_id), nickname: m.card || m.nickname, remaining: m.shut_up_timestamp - now }));
    ...
    server.get('/qqgm/api/stats/messages', async (kctx) => {
        var _a;
        const token = String(kctx.request.headers['x-qqgm-token'] || '');
        const data = verifyToken(token);
        if (!data)
            return kctx.body = { ok: false, message: '未登录' };
        const msgStats = ctx.qqgmMessageStats || new Map();
        const groupStats = msgStats.get(data.groupId) || {};
        const bot = ctx.bots.find(b => b.platform === 'onebot');
        let memberMap = new Map();
        if ((_a = bot === null || bot === void 0 ? void 0 : bot.internal) === null || _a === void 0 ? void 0 : _a.getGroupMemberList) {
            try {
                const ms = await bot.internal.getGroupMemberList(data.groupId);
                for (const m of ms)
                    memberMap.set(String(m.user_id), m.card || m.nickname);
            }
            catch { }
        }
        const stats = Object.entries(groupStats).map(([userId, d]) => ({
    ...
    server.get('/qqgm', (kctx) => { kctx.type = 'text/html'; kctx.body = getAdminPageHtml(); });
    server.get('/qqgm/login/:token', (kctx) => {
